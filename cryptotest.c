...
int main(void) {
    ...
    // -------------------------------------------------------------------
    int type = EVP_PKEY_id(pubkey); // EVP_PKEY_RSA is type 6
    printf("type %d\n", type);
...
//    len = i2d_PublicKey(pubkey, (unsigned char**) &p);
    printf("p %p\n", p);
    printf("public keylen %d\n", len);

    p = b;
//    EVP_PKEY* rec_key = d2i_PublicKey(EVP_PKEY_RSA, NULL, (const unsigned char**) &p, len);
//    EVP_PKEY* rec_key = d2i_PublicKey(type, NULL, (const unsigned char**) &p, len);
    EVP_PKEY* rec_key = d2i_PUBKEY(NULL, (const unsigned char**) &p, len);
    printf("rec_key %p\n", rec_key);
